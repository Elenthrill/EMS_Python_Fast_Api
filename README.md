...
> - **конфигурация** (`config.py`) – параметры подключения к БД, секреты JWT, настройки окружения;  
> - **безопасность** (`security.py`) – хеширование паролей, создание и проверка JWT-токенов;  
> - **подключение к БД** (`database.py`) – создание асинхронного движка SQLAlchemy и фабрики сессий;  
> - **общие зависимости** (`dependencies.py`) – провайдеры для внедрения репозиториев, сервисов и получения текущего пользователя.  
...
> Отделение DTO от доменных сущностей защищает внутреннюю модель от изменений в API и упрощает документирование через Swagger.
